  ...
  function [PSC] = ScatteringRatev3gpu2(CGTable,cteArray,LaserInfoMat,AtomInfoMat,auxArray)
    %Constants
    G =cteArray(9:11);
    kred =  cteArray(4);      
    ...
    %ZeemanShifts 
    auxVec = [1,-1,0];
    MagneticField =G.*AtomInfoMat(:,1:3);
    ZeemanDetunings =magneticMomentTransition *norm(MagneticField)*(AtomInfoMat(:,8)+auxVec);                
