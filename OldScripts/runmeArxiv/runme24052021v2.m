        ...
        %Format above objes to vecotorizable elements.
...
for i = 1:1000000
    ...
    if (atom.AtomicState == 0)
        ...
        ScatteringRateMatrixor  =cell2mat(arrayfun(@(Laser)ScatteringRate(atom,Laser),LaserArray','UniformOutput',false));
        [ScatteringRateMatrix] = atom.ScatteringRatev2(MOTLasersFormat.LaserDetuningMatrix,MOTLasersFormat.BeamPropagationMatrix,MOTLasersFormat.polarizationArray,MOTLasersFormat);
       stoopbool =  abs(ScatteringRateMatrix(2,:) - ScatteringRateMatrixor(2,:))>10;
       if(sum(stoopbool) ~= 0)
           abs(ScatteringRateMatrix(2,2) - ScatteringRateMatrixor(2,2))
       end
